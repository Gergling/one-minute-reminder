...
// const RECORDING_STORAGE_KEY = 'recording-storage-key';

// const getURI = () => AsyncStorage.getItem(RECORDING_STORAGE_KEY);
...
export const useAudio = () => {
  ...
  //   item: storedURI,
  //   setItem: setURI,
  // } = useAsyncStorage(RECORDING_STORAGE_KEY);
  ...
  useEffect(() => {
    if (playing && current === 'idle') {
      dispatch('play');
    }
  }, [current, dispatch, playing]);
